    ...
    public class ReplyHandler : SerializableConfiguration<List<DataTypeS>>, IMessageHandler
    {
        ...
        public async Task<bool> OnMessage(HandlerArgs args)
        {
            ...
            if (isme)
            {
                ...
                if (pending.Length > 0)
                    await args.Callback(FitReply(pending[Utils.rand.Next(pending.Length)], args.Sender));
            ...
            else
            {
                ...
                if (pa.Length > 0)
                {
                    try
                    {
                        await args.Callback(pa[Utils.rand.Next(pa.Length)]());
                    }
                    catch
                    {
                        return true;
        ...
        public async Task ReloadAssembly()
        {
            ...
            {
                "HinaBot_NeoAspect",
                "HinaBot_NeoAspect.Config",
                "HinaBot_NeoAspect.Handler",
                "HinaBot_NeoAspect.Services",
                "System",
                "System.Net.Http",
            ...
            var cfg = GetConfig<AssemblyUsingsConfig>();
            var usings = cfg.list;
            if (usings.Count == 0)
            {
                usings = cfg.list = new List<string>() {
                    "HinaBot_NeoAspect",
                    "HinaBot_NeoAspect.Config",
                    "HinaBot_NeoAspect.Handler",
                    "HinaBot_NeoAspect.Services",
                    "HinaBot_NeoAspect.Models",
                    "System",
                    "System.Net.Http",
                    "System.Collections.Generic",
                    "System.Text.RegularExpressions",
